<template>
  <div class="d-flex flex-column flex-grow-1">
   <div class="d-flex align-center py-3">
    ...
    <v-spacer></v-spacer>
    <v-btn v-if="canCreateCustomerUser || canCreateKoreUser" color="primary" to="add" :append="true">
      Create User
    </v-btn>
   </div>
    ...
    >
      ...
      <template v-slot:item.email="{ item }">
       <div class="d-flex align-center py-1">
        <v-avatar
          ...
          size="28"
        >
          <span class="white--text">{{ item.username | initials }}</span>
        </v-avatar>
      ...
      <template v-slot:item.roleId="{ item }">
       <v-chip
        ...
        dark
        :color="roleColor(item.roleId)"
       >{{ roleName(item.roleId) | capitalize }}</v-chip>
      </template>
      ...
      <template v-slot:item.action="{ item }">
       <div v-if="canCreateCustomerUser || canCreateKoreUser" class="actions">
...
export default {
  ...
  computed: {
   ...mapState({
    ...
    loadingUsersTable: (state) => state.users.loadingUsersTable
   }),
   ...mapGetters({
    roleName: 'auth/roleName',
    canCreateCustomerUser: 'auth/canCreateCustomerUser',
    canCreateKoreUser: 'auth/canCreateKoreUser'
   })
  },
  mounted() {
   if ([1, 2, 3].includes(this.userRole))
    this.getUsers('kore')
   else if ([4, 5, 6].includes(this.userRole))
    this.getUsers('customers')
  },
  methods: {
   ...mapActions({
   ...
   roleColor(roleId) {
    if (roleId === 4 || roleId === 1) return 'primary'
    else if (roleId === 5) return '#4CAF50'
    else if (roleId === 2) return '#4CAF50'
    else return '#F79803'
   }
  }
}
